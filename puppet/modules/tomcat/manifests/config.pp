...
class tomcat::config(
  ...
  $cert_dname = 'CN=shibboleth.vagrant.dev, OU=vagrant.dev, O=vagrant,
L=Chicago, S=Illinois, C=US'

  exec { 'genkeypair':
    command => "keytool -genkeypair -alias tomcat -keyalg RSA -keysize 2048
-dname '${cert_dname}' -storepass changeit -keypass changeit",
    user    => 'tomcat6',
